...
#include <zuri_env/zuri_env.h>
#include <zuri_tooling/zuri_config.h>

tempo_utils::Status
zuri_env::zuri_env(int argc, const char *argv[])
{
    ...
    subcommands[Create] = {"create", "Create a new environment"};
    ...
    if (status.notOk()) {
        ...
        switch (commandStatus.getCondition()) {
            case tempo_command::CommandCondition::kHelpRequested:
                display_help_and_exit({"zuri-env"},
                    "Manage Zuri environments",
    ...
    switch (selected) {
        case Create:
            return env_create_command(zuriConfig, tokens);
        default:
