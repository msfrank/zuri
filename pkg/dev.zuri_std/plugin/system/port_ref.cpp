    ...
    lyric_runtime::InterpreterState *state)
{
    ...
    TU_ASSERT (descriptor.data.descriptor->getSegmentIndex() == currentCoro->peekSP()->getSegmentIndex());
    ...
    if (!subroutineManager->callStatic(descriptor.data.descriptor->getDescriptorIndex(), args, currentCoro, status))
        return lyric_runtime::InterpreterStatus::forCondition(
