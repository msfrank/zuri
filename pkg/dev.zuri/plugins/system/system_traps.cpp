...
public:
    explicit WorkerOperations(lyric_runtime::Task *task)
        : m_task(task)
    {
        TU_NOTNULL (m_task);
    }
        ...
        lyric_runtime::InterpreterState *state) override
    {
        ...
        TU_LOG_INFO << "destroying worker task " << m_task;
        scheduler->destroyTask(m_task);
    }
private:
    lyric_runtime::Task *m_task;
};
    ...
    const lyric_runtime::VirtualTable *unused)
{
    ...
    auto ops = std::make_unique<WorkerOperations>(workerTask);
    auto promise = lyric_runtime::Promise::create(std::move(ops));
